...
import { createEffect, createMemo, createSignal } from '@pulsar-framework/pulsar.dev/reactivity';
import { Button, ComponentConfigBuilder, ComponentStylingBuilder } from '@pulsar-framework/ui';
...
export const BasicSignal: Story = {
  render: () => {
    ...
    desc.textContent =
      '⚡ Click the button to update the signal. Watch how ONLY the count updates, not the entire component!';
    container.appendChild(desc);

    // Count display
    const countContainer = document.createElement('div');
    countContainer.style.cssText =
      'background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 30px; border-radius: 12px; margin: 20px 0; text-align: center;';
...
export const MemoizedComputation: Story = {
  render: () => {
    ...
    desc.textContent =
      '🧮 Memos cache expensive computations. Try accessing factorial multiple times - it only computes once per value!';
    container.appendChild(desc);
    ...
    input.max = '20';
    input.style.cssText =
      'padding: 10px; font-size: 1rem; border: 2px solid #ddd; border-radius: 8px;';
    input.oninput = (e) => {
    ...
    resultContainer.style.cssText =
      'background: #e8f5e9; padding: 20px; border-radius: 12px; margin: 20px 0;';
...
export const DependencyTracking: Story = {
  render: () => {
    ...
    desc.textContent =
      "🎯 Pulsar automatically tracks which signals each memo depends on. Change age - fullName memo won't recompute!";
    container.appendChild(desc);
    ...
    const createInput = (label: string, signal: any, setter: any) => {
      ...
      input.style.cssText =
        'padding: 8px; width: 100%; border: 2px solid #ddd; border-radius: 4px;';
