...
import { createEffect, createMemo, createSignal } from '@pulsar-framework/pulsar.dev';
import { useEffect, useState } from '@pulsar-framework/pulsar.dev/hooks';
import {
  Button,
...
export const UseStateDemo: Story = {
  render: () => {
    ...
    counterSection.style.cssText =
      'background: white; padding: 20px; border-radius: 8px; margin: 15px 0; border: 2px solid #e0e0e0;';
    ...
    counterDisplay.style.cssText =
      'font-size: 2rem; margin: 15px 0; color: #667eea; font-weight: bold;';
    ...
    const renderItemsList = () => {
      ...
      items().forEach((item, index) => {
        ...
        itemEl.style.cssText =
          'display: flex; align-items: center; justify-content: space-between; padding: 10px; margin: 5px 0; background: #f5f5f5; border-radius: 4px;';
...
export const UseEffectDemo: Story = {
  render: () => {
    ...
    desc.textContent =
      '⚡ useEffect runs side effects and supports cleanup functions. Watch the logs!';
    container.appendChild(desc);

    // Timer display
    const timerDisplay = document.createElement('div');
    timerDisplay.style.cssText =
      'background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 40px; border-radius: 12px; margin: 20px 0; text-align: center;';
    ...
    logsSection.style.cssText =
      'background: #f5f5f5; padding: 20px; border-radius: 8px; margin: 20px 0; max-height: 300px; overflow-y: auto;';
    ...
    const renderLogs = () => {
      logsList.innerHTML =
        logs()
          .reverse()
          .map((log) => `<div style="padding: 5px; border-bottom: 1px solid #ddd;">${log}</div>`)
...
export const UseMemoDemo: Story = {
  render: () => {
    ...
    desc.textContent =
      '🧮 useMemo caches expensive computations. Only recalculates when dependencies change.';
    container.appendChild(desc);

    // Stats
    const stats = document.createElement('div');
    stats.style.cssText =
      'background: #f3e5f5; padding: 20px; border-radius: 12px; margin: 20px 0;';
    ...
    const updateUI = () => {
      ...
      buttons.forEach((btn, idx) => {
        ...
        if (isActive) {
          btn.style.cssText =
            'background: #667eea; color: white; border: none; padding: 8px 16px; border-radius: 6px; cursor: pointer; font-weight: 600;';
        } else {
          btn.style.cssText =
            'background: #e2e8f0; color: #4a5568; border: none; padding: 8px 16px; border-radius: 6px; cursor: pointer;';
        }
      });
      ...
      filtered.forEach((item) => {
        ...
        badge.style.cssText =
          'background: #667eea; color: white; padding: 15px 25px; border-radius: 8px; font-size: 1.2rem; font-weight: 600; min-width: 50px; text-align: center;';
