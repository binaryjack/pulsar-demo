...
export const KeyedListUpdates: Story = {
  render: () => {
    ...
    desc.textContent =
      '⚡ Toggle ANY checkbox and watch ONLY that item flash yellow! This proves fine-grained reactivity.';
...
export const ConditionalRendering: Story = {
  render: () => {
    ...
    desc.textContent =
      '🔀 Pulsar handles conditional rendering efficiently. Toggle states to see different UI.';
    container.appendChild(desc);
    ...
    const renderContent = () => {
      ...
      if (!isLoggedIn()) {
        ...
        loginPrompt.style.cssText =
          'background: #fff3cd; padding: 30px; border-radius: 12px; text-align: center;';
