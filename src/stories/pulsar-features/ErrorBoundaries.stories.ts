import { Catcher, Tryer } from '@pulsar-framework/pulsar.dev/error-boundary';
import {
  Badge,
  Button,
  Card,
...
export const BasicErrorHandling: Story = {
  render: () => {
    ...
    desc.style.cssText =
      'background: #fee; padding: 15px; border-radius: 8px; margin: 15px 0; border: 2px solid #dc2626;';
    desc.textContent =
      '💥 Click "Throw Error" to simulate a component crash. The error boundary will catch it gracefully!';
    ...
    warningNote.style.cssText =
      'margin-top: 20px; padding: 10px; background: #fffbeb; border-left: 4px solid #f59e0b; color: #92400e; font-size: 14px;';
    warningNote.textContent =
      'Note: "Catcher requires Tryer parent" warning is expected - this demo simulates error states manually for demonstration purposes.';
...
export const MultipleErrorBoundaries: Story = {
  render: () => {
    ...
    desc.textContent =
      '🛡️ Each widget has its own error boundary. If one crashes, others keep working!';
    container.appendChild(desc);
    ...
    notice.style.cssText =
      'background: #f0fdf4; padding: 15px; border-radius: 8px; margin: 20px 0; border: 1px solid #059669;';
    notice.innerHTML = `
