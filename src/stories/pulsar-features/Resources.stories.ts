...
import { createResource } from '@pulsar-framework/pulsar.dev/resource';
import {
  Badge,
  Button,
  Card,
...
export const BasicResourceFetching: Story = {
  render: () => {
    ...
    desc.textContent =
      '🔄 Click buttons to fetch different users. Notice the automatic loading states!';
...
export const ParallelResourceLoading: Story = {
  render: () => {
    ...
    desc.textContent =
      '⚡ Loading user profile and posts in parallel. Watch both resources load independently!';
    container.appendChild(desc);
